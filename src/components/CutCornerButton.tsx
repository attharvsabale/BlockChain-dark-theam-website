...
export const CutCornerButton = (props: ComponentPropsWithoutRef<'button'>) => {
  const { className, children, ...rest } = props; // Spread other props

  return (
    <button
      className={twMerge(
        "bg-fuchsia-500/20 px-4 py-2 font-extrabold uppercase font-heading text-sm tracking-wide relative overflow-hidden", // Merged and cleaned up classes
        className
      )}
    >
      <div
      ...
      >
        <path
          ...
          stroke="currentColor"
          strokeWidth="2" // Changed `stroke-width` to `strokeWidth` (JSX format)
        ></path>
      </svg>
